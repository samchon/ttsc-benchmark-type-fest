...

{
  ...
  type Type = Array<{ x: string }> &
    Array<{ z: number; d: { e: string; f: boolean } }>;
  const function_ = <T extends Exact<Type, T>>(arguments_: T) => arguments_;
...
// @see https://github.com/sindresorhus/type-fest/issues/508
{
  ...
  const onlyAcceptNameImproved = <T extends Exact<OnlyAcceptName, T>>(
    arguments_: T,
  ) => arguments_;
...
// @see https://github.com/sindresorhus/type-fest/issues/508
{
  ...
  const function_ = <T extends Exact<OnlyAcceptName, T>>(arguments_: T) =>
    arguments_;
...
// Spec - test the above for tagged types too.
{
  ...
  const function_ = <T extends Exact<{ a: TaggedNumber }, T>>(arguments_: T) =>
    arguments_;
...
// https://github.com/sindresorhus/type-fest/issues/545
{
  ...
  const function_ = <InputT extends Exact<ParameterType, InputT>>(
    arguments_: InputT,
  ) => arguments_;
