...
expectType<[x: [x1: string, x2: string], y: [y1: string, y2: string]]>(
  {} as Schema<[x: [x1: number, x2: number], y: [y1: number, y2: number]], string>,
);
// Caveat: Optional properties with explicit `undefined` don't behave differently from optional properties without explicit `undefined`.
...
expectType<{a: number; b: number} | number>(
  {} as Schema<{a: string[]; b: string} | [string, string, string], number, {recurseIntoArrays: false}>,
);
expectType<{a: 'a'; b: 'a'}>(
  {} as Schema<{a: number[]; b: [string, string]}, 'a', {recurseIntoArrays: false}>,
);
expectType<{a: {b: {c: {d: {e: string[]}}}}}>(
  {} as Schema<{a: {b: {c: {d: string[]}}}}, {e: string[]}, {recurseIntoArrays: false}>,
);
