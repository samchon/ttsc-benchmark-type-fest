...
expectType<'1' | 'foo'>(
  {} as KeyAsString<{[sym]: unknown; 1: number; foo: string}>,
); // Ignores symbol keys
...
expectType<`foo${string}` | `${number}`>(
  {} as KeyAsString<{[x: `foo${string}`]: unknown; [y: number]: unknown}>,
);
expectType<string>(
  {} as KeyAsString<{[x: string]: unknown; [y: symbol]: unknown}>,
);
expectType<never>({} as KeyAsString<{[x: symbol]: unknown}>);

// Unions
expectType<'1'>(
  {} as KeyAsString<{1: number; foo: string} | {1: number; bar: string}>,
); // Only grabs the common keys, just like `keyof`.
