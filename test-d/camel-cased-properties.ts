...
declare const baz: CamelCasedProperties<
  { fooBAR: number; BARFoo: string },
...
declare const fooBarPunctuated: CamelCasedProperties<{
  'hello@world1': { 'foo::bar': string };
}>;
expectType<{ 'hello@world1': { 'foo::bar': string } }>(fooBarPunctuated);

declare const fooBarPunctuatedSplit: CamelCasedProperties<
  { 'hello@world1': { 'foo::bar': string } },
  { splitOnPunctuation: true }
>;
expectType<{ helloWorld1: { 'foo::bar': string } }>(fooBarPunctuatedSplit);

declare const fooBarPunctuatedSplitNumberSplit: CamelCasedProperties<
  { 'hello@world1': { 'foo::bar': string } },
...
expectType<CamelCasedProperties<UserPunctuated, { splitOnPunctuation: true }>>(
  result,
);

declare const withLeadingUnderscores: CamelCasedProperties<
  { _foo_bar: string; __baz_qux: number },
  { preserveLeadingUnderscores: true }
>;
expectType<{ _fooBar: string; __bazQux: number }>(withLeadingUnderscores);

declare const withLeadingUnderscoresDefault: CamelCasedProperties<{
  _foo_bar: string;
}>;
