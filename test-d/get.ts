...
declare const get: <ObjectType, Path extends string | readonly string[]>(
  object: ObjectType,
  path: Path,
) => Get<ObjectType, Path, NonStrict>;
...
expectTypeOf(
  get(withDictionary, ['baz', 'something', 'qux', '0', 'x'] as const),
).toBeBoolean();

// Should work with dynamic keys
declare const someKey: string;
expectTypeOf(
  get(withDictionary, ['foo', someKey, 'bar'] as const),
).toBeNumber();
...
expectTypeOf<Get<WithModifiers, 'foo[0].bar.baz', NonStrict>>().toEqualTypeOf<
  { qux: number } | undefined
>();
expectTypeOf<
  Get<WithModifiers, 'foo[0].abc.def.ghi', NonStrict>
...
expectTypeOf<
  Get<
    { a: { b: Array<Array<Array<{ id: number }>>> } },
    'a.b[0][0][0].id',
    NonStrict
  >
>().toBeNumber();
...
expectTypeOf<Get<Record<string, number>, 'foo'>>().toEqualTypeOf<
  number | undefined
>();
...
expectTypeOf<Get<WithDictionary, 'foo.whatever'>>().toEqualTypeOf<
  { bar: number } | undefined
>();
expectTypeOf<Get<WithDictionary, 'foo.whatever.bar'>>().toEqualTypeOf<
  number | undefined
>();
expectTypeOf<Get<WithDictionary, 'baz.whatever.qux[3].x'>>().toEqualTypeOf<
  boolean | undefined
>();
expectTypeOf<
  Get<WithDictionary, ['baz', 'whatever', 'qux', '3', 'x']>
...
expectTypeOf<WithDictionary>().toEqualTypeOf<
  Get<WithDictionary, readonly []>
>();
