...
expectType<'a' | 'c'>(
  {} as ReadonlyKeysOf<{readonly a?: string; b: number; readonly c: boolean}>,
);
expectType<'c'>(
  {} as ReadonlyKeysOf<{a?: string; b: number; readonly c: boolean}>,
);

// Unions
expectType<'b' | 'c'>(
  {} as ReadonlyKeysOf<{a: string; readonly b: number} | {readonly c?: string; d?: number}>,
);
expectType<'a' | 'b'>(
  {} as ReadonlyKeysOf<{a: string; b: number} | {readonly a: string; readonly b: number}>,
);

// Arrays
expectType<never>(
  {} as Extract<ReadonlyKeysOf<[string, number, boolean]>, number | `${number}`>,
);
expectType<number | '0' | '1' | '2'>(
  {} as Extract<ReadonlyKeysOf<readonly [string, number, boolean]>, number | `${number}`>,
);
expectType<never>(
  {} as Extract<ReadonlyKeysOf<string[]>, number | `${number}`>,
);
expectType<number>(
  {} as Extract<ReadonlyKeysOf<readonly string[]>, number | `${number}`>,
);
