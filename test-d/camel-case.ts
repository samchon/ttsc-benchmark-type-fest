...
const camelFromVendorPrefixedCssProperty: CamelCase<'-webkit-animation'> =
  'webkitAnimation';
expectType<'webkitAnimation'>(camelFromVendorPrefixedCssProperty);

const camelFromDoublePrefixedKebab: CamelCase<'--very-prefixed'> =
  'veryPrefixed';
...
expectType<CamelCase<'fooBAR', { preserveConsecutiveUppercase: true }>>(
  'fooBAR',
);

expectType<CamelCase<'fooBARBiz'>>('fooBarBiz');
expectType<CamelCase<'fooBARBiz', { preserveConsecutiveUppercase: true }>>(
  'fooBARBiz',
);

expectType<
  CamelCase<'foo BAR-Biz_BUZZ', { preserveConsecutiveUppercase: true }>
>('fooBARBizBUZZ');
expectType<
  CamelCase<'foo BAR-Biz_BUZZ', { preserveConsecutiveUppercase: false }>
>('fooBarBizBuzz');
expectType<CamelCase<'foo\tBAR-Biz_BUZZ'>>('fooBarBizBuzz');

expectType<CamelCase<string, { preserveConsecutiveUppercase: true }>>(
  'string' as string,
);
...
expectType<'p2pNetwork'>(
  '' as CamelCase<'p2pNetwork', { splitOnNumbers: false }>,
);
expectType<'p2PNetwork'>(
  '' as CamelCase<'p2pNetwork', { splitOnNumbers: true }>,
);
...
expectType<CamelCase<'onDialog:close', { splitOnPunctuation: true }>>(
  'onDialogClose',
);
expectType<CamelCase<'foo-bar>>baz', { splitOnPunctuation: true }>>(
  'fooBarBaz',
);
expectType<
  CamelCase<
    'fooBAR:biz',
    { splitOnPunctuation: true; preserveConsecutiveUppercase: true }
  >
>('fooBARBiz');
expectType<CamelCase<'foo-bar::01', { splitOnPunctuation: true }>>('fooBar01');
expectType<
  CamelCase<'foo-bar::01', { splitOnPunctuation: true; splitOnNumbers: false }>
>('fooBar01');
expectType<
  CamelCase<'foo-bar::01', { splitOnPunctuation: true; splitOnNumbers: true }>
>('fooBar01');

expectType<CamelCase<'foo_bar', { preserveLeadingUnderscores: true }>>(
  'fooBar',
);
expectType<CamelCase<'_foo_bar', { preserveLeadingUnderscores: true }>>(
  '_fooBar',
);
expectType<CamelCase<'__foo_bar', { preserveLeadingUnderscores: true }>>(
  '__fooBar',
);
expectType<CamelCase<'_FOO_BAR', { preserveLeadingUnderscores: true }>>(
  '_fooBar',
...
expectType<CamelCase<'_foo-bar', { preserveLeadingUnderscores: true }>>(
  '_fooBar',
);
expectType<
  CamelCase<
    '_fooBAR',
    { preserveLeadingUnderscores: true; preserveConsecutiveUppercase: true }
  >
>('_fooBAR');
expectType<CamelCase<'_foo_bar'>>('fooBar');
expectType<CamelCase<'_foo_bar', { preserveLeadingUnderscores: false }>>(
  'fooBar',
);
