...
expectType<PascalCase<'fooBAR', {preserveConsecutiveUppercase: true}>>(
  'FooBAR',
);

expectType<PascalCase<'fooBARBiz'>>('FooBarBiz');
expectType<PascalCase<'fooBARBiz', {preserveConsecutiveUppercase: true}>>(
  'FooBARBiz',
);

expectType<PascalCase<'foo BAR-Biz_BUZZ', {preserveConsecutiveUppercase: true}>>(
  'FooBARBizBUZZ',
);
expectType<PascalCase<'foo BAR-Biz_BUZZ', {preserveConsecutiveUppercase: false}>>(
  'FooBarBizBuzz',
);
expectType<PascalCase<'foo\tBAR-Biz_BUZZ'>>('FooBarBizBuzz');

expectType<PascalCase<string, {preserveConsecutiveUppercase: true}>>(
...
expectType<PascalCase<'onDialog:close', {splitOnPunctuation: true}>>(
  'OnDialogClose',
);
expectType<PascalCase<'foo-bar>>baz', {splitOnPunctuation: true}>>('FooBarBaz');
expectType<PascalCase<'fooBAR:biz', {splitOnPunctuation: true; preserveConsecutiveUppercase: true}>>(
  'FooBARBiz',
);
expectType<PascalCase<'foo-bar::01', {splitOnPunctuation: true}>>('FooBar01');
expectType<PascalCase<'foo-bar::01', {splitOnPunctuation: true; splitOnNumbers: false}>>(
  'FooBar01',
);
expectType<PascalCase<'foo-bar::01', {splitOnPunctuation: true; splitOnNumbers: true}>>(
  'FooBar01',
);
