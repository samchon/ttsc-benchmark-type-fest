...
expectType<{ readonly a: 'life'; b: number }>(
  mergeDeep({ a: 'life' } as const, { b: 42 }),
);
expectType<{ a: string; readonly b: 42 }>(
  mergeDeep({ a: 'life' }, { b: 42 } as const),
);
expectType<{ readonly a: 'life'; readonly b: 42 }>(
  mergeDeep({ a: 'life' } as const, { b: 42 } as const),
);
...
expectType<Array<string | number>>(
  mergeDeep(['life'], [42], { arrayMergeMode: 'spread' }),
);
expectType<Array<'life' | number>>(
  mergeDeep(['life'] as const, [42], { arrayMergeMode: 'spread' }),
);
expectType<Array<string | 42>>(
  mergeDeep(['life'], [42] as const, { arrayMergeMode: 'spread' }),
);
expectType<Array<'life' | 42>>(
  mergeDeep(['life'] as const, [42] as const, { arrayMergeMode: 'spread' }),
);

// Should replace simple arrays/tuples
expectType<Array<string | number>>(
  mergeDeep(['life'], [42], { arrayMergeMode: 'replace' }),
);
expectType<Array<'life' | number>>(
  mergeDeep(['life'] as const, [42], { arrayMergeMode: 'replace' }),
);
expectType<Array<string | 42>>(
  mergeDeep(['life'], [42] as const, { arrayMergeMode: 'replace' }),
);
expectType<Array<'life' | 42>>(
  mergeDeep(['life'] as const, [42] as const, { arrayMergeMode: 'replace' }),
);

// Should merge tuples with union
expectType<Array<number | string | boolean>>(
  mergeDeep(['life', true], [42], { arrayMergeMode: 'spread' }),
);
expectType<Array<number | string | true>>(
  mergeDeep(['life'], [42, true], { arrayMergeMode: 'spread' }),
);
...
expectType<{ foo: string; bar: number; fooBar: boolean; items: number[] }>(
  fooBar,
);
...
type BarWithIndexSignature = {
  [x: symbol]: symbol;
  bar: number;
  items: number[];
};
type FooWithIndexSignatureDeep = {
  [x: number]: number;
  foo: string;
  fooBar: FooWithIndexSignature;
  items: string[];
};
type BarWithIndexSignatureDeep = {
  [x: symbol]: symbol;
  bar: number;
  fooBar: BarWithIndexSignature;
  items: number[];
};

declare const fooBarWithIndexSignature: MergeDeep<
  FooWithIndexSignatureDeep,
  BarWithIndexSignatureDeep
>;
expectType<{
  [x: number]: number;
  [x: symbol]: symbol;
...
type FooWithOptional = {
  foo: string;
  fooOptional?: string;
  fooBar: Foo;
  fooBarOptional: Foo | undefined;
};
type BarWithOptional = {
  bar: number;
  barOptional?: number;
  fooBar: Bar;
  fooBarOptional: Bar | undefined;
};
...
declare const fooBarArraySpread: MergeDeep<
  FooArray,
  BarArray,
...
declare const fooBarArrayReplace: MergeDeep<
  FooArray,
  BarArray,
...
declare const fooBarArraySpreadRecursive: MergeDeep<
  FooArray,
  BarArray,
...
declare const fooBarArrayReplaceRecursive: MergeDeep<
  FooArray,
  BarArray,
  { arrayMergeMode: 'replace'; recurseIntoArrays: true }
>;
expectType<
  Array<{
    foo: string;
...
declare const fooBarArraySpreadRecursiveFallback: MergeDeep<
  FooArray,
  string[],
  { arrayMergeMode: 'spread'; recurseIntoArrays: true }
>;
...
declare const fooBarArrayReplaceRecursiveFallback: MergeDeep<
  FooArray,
  string[],
  { arrayMergeMode: 'replace'; recurseIntoArrays: true }
>;
...
declare const fooBarArrayDeepUnionRecursive: MergeDeep<
  FooArray[][],
  BarArray[][],
  { arrayMergeMode: 'spread'; recurseIntoArrays: true }
>;
...
declare const fooBarArrayDeepUnionRecursiveFallback: MergeDeep<
  FooArray[],
  BarArray[][],
  { arrayMergeMode: 'spread'; recurseIntoArrays: true }
>;
...
declare const fooBarTupleSpread: MergeDeep<
  FooTuple,
  BarTuple,
  { arrayMergeMode: 'spread'; recurseIntoArrays: true }
>;
expectType<[FooBarSpread, [FooBarSpread[], 'a', 'b'], 'bar', true]>(
  fooBarTupleSpread,
);

declare const fooBarTupleReplace: MergeDeep<
  FooTuple,
  BarTuple,
  { arrayMergeMode: 'replace'; recurseIntoArrays: true }
>;
expectType<[FooBarReplace, [FooBarReplace[], 'a', 'b'], 'bar', true]>(
  fooBarTupleReplace,
);
...
declare const fooNumberTupleBarArray2DSpread: MergeDeep<
  FooNumberTuple,
  BarArray2D,
  { arrayMergeMode: 'spread'; recurseIntoArrays: true }
>;
expectType<[FooBarSpread[], Array<number | Bar>, ...BarArray2D]>(
  fooNumberTupleBarArray2DSpread,
);

declare const fooNumberTupleBarArray2DReplace: MergeDeep<
  FooNumberTuple,
  BarArray2D,
  { arrayMergeMode: 'replace'; recurseIntoArrays: true }
>;
expectType<[FooBarReplace[], Bar[], ...BarArray2D]>(
  fooNumberTupleBarArray2DReplace,
);
...
declare const fooArray2DBarNumberTupleSpread: MergeDeep<
  FooArray2D,
  BarNumberTuple,
  { arrayMergeMode: 'spread'; recurseIntoArrays: true }
>;
expectType<[FooBarSpread[], Array<Foo | number>, ...FooArray2D]>(
  fooArray2DBarNumberTupleSpread,
);

declare const fooArray2DBarNumberTupleReplace: MergeDeep<
  FooArray2D,
  BarNumberTuple,
  { arrayMergeMode: 'replace'; recurseIntoArrays: true }
>;
expectType<[FooBarReplace[], number[], ...FooArray2D]>(
  fooArray2DBarNumberTupleReplace,
);

// Should merge array into tuple with object entries and variadic length
declare const arrayIntoTupleWithVariadicSpread: MergeDeep<
  [number, Foo, ...Foo[]],
  Bar[],
  { arrayMergeMode: 'spread'; recurseIntoArrays: true }
>;
expectType<[Bar, FooBarSpread, ...FooBarSpread[]]>(
  arrayIntoTupleWithVariadicSpread,
);

declare const arrayIntoTupleWithVariadicReplace: MergeDeep<
  [number, Foo, ...Foo[]],
  Bar[],
  { arrayMergeMode: 'replace'; recurseIntoArrays: true }
>;
expectType<[Bar, FooBarReplace, ...FooBarReplace[]]>(
  arrayIntoTupleWithVariadicReplace,
);

// Should merge tuple into array with object entries and variadic length
declare const tupleIntoArrayWithVariadicSpread: MergeDeep<
  Foo[],
  [number, Bar, ...Bar[]],
  { arrayMergeMode: 'spread'; recurseIntoArrays: true }
>;
expectType<[number, FooBarSpread, ...FooBarSpread[]]>(
  tupleIntoArrayWithVariadicSpread,
);

declare const tupleIntoArrayWithVariadicReplace: MergeDeep<
  Foo[],
  [number, Bar, ...Bar[]],
  { arrayMergeMode: 'replace'; recurseIntoArrays: true }
>;
expectType<[number, FooBarReplace, ...FooBarReplace[]]>(
  tupleIntoArrayWithVariadicReplace,
);

// Should merge tuple into tuple with object entries and variadic length
declare const tupleIntoTupleWithVariadicSpread: MergeDeep<
  [number, ...Foo[]],
  [Bar, Bar, ...Bar[]],
  { arrayMergeMode: 'spread'; recurseIntoArrays: true }
>;
expectType<[Bar, FooBarSpread, ...FooBarSpread[]]>(
  tupleIntoTupleWithVariadicSpread,
);

declare const tupleIntoTupleWithVariadicSpreadReversed: MergeDeep<
  [Foo, ...Foo[]],
  [number, Bar, ...Bar[]],
  { arrayMergeMode: 'spread'; recurseIntoArrays: true }
>;
...
declare const tupleIntoTupleWithVariadicReplace: MergeDeep<
  [number, ...Foo[]],
  [Bar, Bar, ...Bar[]],
  { arrayMergeMode: 'replace'; recurseIntoArrays: true }
>;
expectType<[Bar, FooBarReplace, ...FooBarReplace[]]>(
  tupleIntoTupleWithVariadicReplace,
);

declare const tupleIntoTupleWithVariadicReplaceReversed: MergeDeep<
  [Foo, ...Foo[]],
  [number, Bar, ...Bar[]],
  { arrayMergeMode: 'replace'; recurseIntoArrays: true }
>;
expectType<[number, FooBarReplace, ...FooBarReplace[]]>(
  tupleIntoTupleWithVariadicReplaceReversed,
);
...
expectType<OptionalWithUndefined>(
  {} as MergeDeep<NotOptional, OptionalWithUndefined>,
);
