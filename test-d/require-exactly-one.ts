...
declare const testWithoutKeys: (
  _: RequireExactlyOne<{ a: number; b: number }>,
) => void;
...
function narrowingTest(
  foo: Simplify<RequireExactlyOne<{ a: string; b: string }>>,
): string {
...
function narrowingTest2(
  foo: Simplify<RequireExactlyOne<{ a: string; b: string; c: string }>>,
): string {
  // `Simplify` just makes it easier to visualize the narrowing
  if (typeof foo.a === 'string') {
    expectAssignable<string>(foo.a);
...
function narrowingTest3(
  foo: Simplify<
    RequireExactlyOne<{ a: string; b: string; c: string }, 'a' | 'b'>
  >,
): string {
  // `Simplify` just makes it easier to visualize the narrowing
  if (typeof foo.a === 'string') {
