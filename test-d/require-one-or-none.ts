...
// 'foo' always required
type OneOrTwo = RequireOneOrNone<
  Record<'foo' | 'bar' | 'baz', true>,
  'bar' | 'baz'
>;
...
function narrowingTest(
  foo: Simplify<RequireOneOrNone<{ a: string; b: string }>>,
): string {
...
function narrowingTest2(
  foo: Simplify<RequireOneOrNone<{ a: string; b: string; c: string }>>,
): string {
  // `Simplify` just makes it easier to visualize the narrowing
  if (typeof foo.a === 'string') {
    expectAssignable<string>(foo.a);
...
function narrowingTest3(
  foo: Simplify<
    RequireOneOrNone<{ a: string; b: string; c: string }, 'a' | 'b'>
  >,
): string {
  // `Simplify` just makes it easier to visualize the narrowing
  if (typeof foo.a === 'string') {
...
expectType<
  { a: number; b?: never } | { a?: never; b: string } | { a?: never; b?: never }
>(
