...
expectType<'handle-click'>(
  {} as RemovePrefix<'handle-click', Uppercase<string>>,
);
...
expectType<`${string}/${number}`>(
  {} as RemovePrefix<`${string}/${number}`, `${string}:`>,
);
expectType<`${number}:${number}`>(
  {} as RemovePrefix<`${number}:${number}`, `-${string}`>,
);

// Unions
expectType<'click' | 'hover' | 'change'>(
  {} as RemovePrefix<'on-click' | 'on-hover' | 'on-change', 'on-'>,
);
expectType<'click' | 'hover' | 'handle-change'>(
  {} as RemovePrefix<'on-click' | 'on-hover' | 'handle-change', 'on-'>,
);
expectType<Uppercase<string> | `${number}`>(
  {} as RemovePrefix<`id-${Uppercase<string>}` | `id-${number}`, 'id-'>,
);
expectType<string>(
  {} as RemovePrefix<`hover:${string}` | `focus:${string}`, `${string}:`>,
);

expectType<'-change' | 'change'>({} as RemovePrefix<'on-change', 'on' | 'on-'>);
expectType<'change' | 'on-change'>(
  {} as RemovePrefix<'on-change', 'on-' | 'handle-'>,
);
expectType<'on-change'>({} as RemovePrefix<'on-change', 'off-' | 'handle-'>);
expectType<string>({} as RemovePrefix<'on-change', `${string}-` | 'on'>);

expectType<'on:change' | 'onChange'>(
  {} as RemovePrefix<'on:change' | 'onChange', `${string}-`>,
);
