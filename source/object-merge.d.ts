...
> = IfNotAnyOrNever<
  ...
  IfNotAnyOrNever<
    Second,
    First extends unknown // For distributing `First`
...
> = Simplify<
  {
    ...
    -readonly [P in keyof Second as P extends NormalizedSecondLiteralKeys
      ...
      | (P extends NormalizedKeys<keyof PickIndexSignature<First>>
          ? If<
              IsOptionalKeyOf<Second, P>,
              First[NormalizedKeys<P> & keyof First],
              never
            >
          : never);
  } & {
    ...
    -readonly [P in keyof Second as P extends NormalizedSecondLiteralKeys
      ...
      | (NormalizedKeys<P> &
          Exclude<
            keyof First,
            NormalizedKeys<RequiredKeysOf<OmitIndexSignature<Second>>>
          > extends infer NonOverwrittenKeysOfFirst
          ? If<
              IsNever<NonOverwrittenKeysOfFirst>, // This check is required because indexing with `never` doesn't always yield `never`, for example, `{[x: string]: number}[never]` results in `number`.
              never,
    ...
    -readonly [P in keyof First as P extends NormalizedFirstLiteralKeys
      ...
      | First[P]
      | If<
          IsNever<NormalizedKeys<P> & keyof Second>, // This check is required because indexing with `never` doesn't always yield `never`, for example, `{[x: string]: number}[never]` results in `number`.
          never,
    ...
    -readonly [P in keyof First as P extends NormalizedFirstLiteralKeys
      ? P extends NormalizedSecondLiteralKeys
        ? If<
            IsOptionalKeyOf<Second, NormalizedKeys<P> & keyof Second>,
            P,
            never
          >
        : never
